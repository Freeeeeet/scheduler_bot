...
func (r *RecurringScheduleRepository) Create(ctx context.Context, schedule *model.RecurringSchedule) error {
	query := `
		INSERT INTO recurring_schedules (teacher_id, subject_id, weekday, start_hour, start_minute, duration_minutes, is_active)
...
func (r *RecurringScheduleRepository) GetByID(ctx context.Context, id int64) (*model.RecurringSchedule, error) {
	query := `
		SELECT id, teacher_id, subject_id, weekday, start_hour, start_minute, duration_minutes, is_active, created_at, updated_at
